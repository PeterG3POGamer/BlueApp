...
class SyncVentaAndDetalle(private val context: Context) {
                    ...
                    enviarVenta(venta, detalleNotSync) { success, error ->
                        if (success) {
                            ...
                            if (result <= 0) {
                                Log.e(
                                    "DataSyncManager",
                                    "Error al actualizar el estado de la venta ${venta.id}"
                                )
                            }
                        } else {
                            Log.e(
                                "DataSyncManager",
                                "Error al sincronizar venta ${venta.id}: ${error?.message}"
                            )
                        }
                    }
                }
    ...
    suspend fun enviarVenta(
        ...
        val baseUrl =
            "${Constants.getBaseUrl()}controllers/PesoPollosController.php?op=InsertarDataPesoPollos"
